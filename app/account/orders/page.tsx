...
export default function OrdersPage() {
      ...
      <div className="min-h-screen bg-cream-100 py-16">
        <div className="container-custom max-w-4xl">
          ...
          {Array.from({ length: 3 }).map((_, i) => (
            <div key={i} className="bg-white rounded-2xl shadow-card p-6 mb-4 animate-pulse">
  ...
  return (
    <div className="bg-cream-100 min-h-screen py-10">
      <div className="container-custom max-w-4xl">
        <h1 className="font-serif text-3xl font-bold text-primary-500 mb-8">My Orders</h1>

        {orders.length === 0 ? (
          <div className="text-center py-20">
            <Package size={48} className="text-gray-300 mx-auto mb-4" />
            <p className="font-serif text-xl text-gray-600 mb-3">No orders yet</p>
            ...
            {orders.map((order, i) => (
              ...
              >
                <div className="flex flex-wrap items-start justify-between gap-4 mb-4">
                  ...
                  <span className={`badge text-sm font-semibold ${STATUS_MAP[order.status]?.color || 'bg-gray-100 text-gray-600'}`}>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  {order.items?.slice(0, 3).map((item) => (
                    <div key={item.id} className="flex items-center gap-2 bg-cream-100 rounded-lg px-3 py-2">
                      <span className="text-sm text-gray-700 truncate max-w-[180px]">{item.product_name}</span>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-gray-100">
                  <p className="font-bold text-xl text-primary-600">{formatPrice(order.total)}</p>
                  <Link href={`/account/orders/${order.id}`} className="flex items-center gap-1.5 text-sm text-primary-500 font-medium hover:text-primary-700 transition-colors">
