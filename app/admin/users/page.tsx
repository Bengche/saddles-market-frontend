...
export default function AdminUsersPage() {
  ...
  return (
    <div className="p-8">
      <h1 className="font-serif text-3xl font-bold text-gray-900 mb-6">Users</h1>
              ...
              {loading ? (
                Array.from({ length: 8 }).map((_, i) => (
                  <tr key={i} className="border-b border-gray-50">
                    <td colSpan={5} className="px-5 py-3"><div className="h-6 bg-gray-100 rounded animate-pulse" /></td>
                  </tr>
                ))
              ) : users.map((user) => (
                <tr key={user.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3">
                    <button onClick={() => toggleActive(user.id, user.is_active)} title={user.is_active ? 'Deactivate' : 'Activate'} className={`w-9 h-5 rounded-full transition-colors relative ${user.is_active ? 'bg-green-500' : 'bg-gray-200'}`}>
                      <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${user.is_active ? 'translate-x-4' : 'translate-x-0.5'}`} />
                    </button>
                  </td>
                  <td className="px-5 py-3">
                    <button onClick={() => toggleAdmin(user.id, user.is_admin)} title={user.is_admin ? 'Revoke admin' : 'Grant admin'} className={`p-1.5 rounded-lg transition-colors ${user.is_admin ? 'bg-gold-100 text-gold-600 hover:bg-gold-200' : 'bg-gray-100 text-gray-400 hover:bg-gray-200'}`}>
