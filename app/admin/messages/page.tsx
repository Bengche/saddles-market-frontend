    ...
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          {unreadCount > 0 && (
            <p className="text-sm text-red-500 mt-0.5">{unreadCount} unread</p>
          )}
        </div>
        <div className="flex gap-2">
          {(["all", "unread"] as const).map((f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
              className={`px-4 py-1.5 rounded-xl text-sm font-medium capitalize transition-colors ${filter === f ? "bg-primary-500 text-white" : "bg-white border border-gray-200 text-gray-500 hover:bg-gray-50"}`}
            >
              {f}
            </button>
          ))}
      ...
      {loading ? (
        ...
        <div className="space-y-2">
          {messages.map((msg) => (
            <div
              key={msg.id}
              className={`bg-white rounded-2xl shadow-sm overflow-hidden ${!msg.is_read ? "border-l-4 border-primary-500" : ""}`}
            >
              <div
              ...
              >
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 mb-0.5">
                    <span
                      className={`font-medium text-sm ${!msg.is_read ? "text-gray-900" : "text-gray-600"}`}
                    >
                      {msg.name}
                    </span>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <ChevronDown
                    size={16}
                    className={`text-gray-400 transition-transform ${expanded === msg.id ? "rotate-180" : ""}`}
                  />
                </div>
              </div>
              {expanded === msg.id && (
                <div className="px-5 pb-5 border-t border-gray-50">
                  <p className="text-gray-600 leading-relaxed mt-4 text-sm whitespace-pre-wrap">
                    {msg.message}
                  </p>
                  {msg.phone && (
