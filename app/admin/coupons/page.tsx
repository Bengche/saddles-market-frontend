...
const couponSchema = z.object({
  code: z.string().min(3, 'Code must be at least 3 chars').max(20).toUpperCase(),
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6">
        ...
        <button onClick={() => setCreating(true)} className="btn-primary flex items-center gap-2 py-2 px-5 text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr key={coupon.id} className="border-b border-gray-50 hover:bg-gray-50">
                  ...
                  <td className="px-5 py-3 text-gray-500">{coupon.min_order_amount ? formatPrice(coupon.min_order_amount) : '—'}</td>
                  <td className="px-5 py-3 text-gray-500">{coupon.usage_count}{coupon.usage_limit ? ` / ${coupon.usage_limit}` : ''}</td>
                  <td className="px-5 py-3 text-gray-400">{coupon.expires_at ? new Date(coupon.expires_at).toLocaleDateString() : '—'}</td>
                  <td className="px-5 py-3">
                    <button onClick={() => toggleActive(coupon.id, coupon.is_active)} className={`w-9 h-5 rounded-full transition-colors relative ${coupon.is_active ? 'bg-green-500' : 'bg-gray-200'}`}>
                      <span className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-transform ${coupon.is_active ? 'translate-x-4' : 'translate-x-0.5'}`} />
                    </button>
                  </td>
                  <td className="px-5 py-3">
                    <button onClick={() => setDeleteId(coupon.id)} className="p-1.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-lg"><Trash2 size={15} /></button>
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      {creating && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-7 w-full max-w-md">
            <h3 className="font-serif text-xl font-semibold text-gray-900 mb-5">Create Coupon</h3>
            <form onSubmit={handleSubmit(createCoupon)} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">Code *</label>
                <input {...register('code')} className="input-field font-mono" placeholder="SUMMER20" />
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Value * ({discountType === 'percentage' ? '%' : '$'})</label>
                  <input type="number" step={discountType === 'percentage' ? '1' : '0.01'} {...register('discount_value')} className="input-field" />
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="number" step="0.01" {...register('min_order_amount')} className="input-field" placeholder="0.00" />
                </div>
                <div>
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setCreating(false)} className="btn-secondary flex-1 py-2">Cancel</button>
                <button type="submit" disabled={isSubmitting} className="btn-primary flex-1 py-2">{isSubmitting ? 'Creating...' : 'Create Coupon'}</button>
              </div>
            </form>
          </div>
        </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
          <div className="bg-white rounded-2xl shadow-2xl p-7 max-w-sm w-full">
            ...
            <div className="flex gap-3">
              ...
              <button onClick={() => deleteCoupon(deleteId)} className="flex-1 py-2 px-4 bg-red-500 hover:bg-red-600 text-white rounded-xl font-medium text-sm transition-colors">Delete</button>
            </div>
          </div>
        </div>
      )}
